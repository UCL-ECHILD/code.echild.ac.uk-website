...
Some code lists have additional columns, such as the [Hardelid et al](chc_hardelid_v1) list of chronic health conditions, which contains columns for the groups and sub-groups (`group` and `subgroup`) into which the codes are divided in Hardelid et al’s [-@hardelid2014] original paper:
...
Not all code lists use ICD-10 or OPCS codes (or exclusively these). For example, the [Zylbersztejn et al](births_zylb_v1) code list used to identify births, in addition to being designed for use where `startage` is `7001` or `7002` (`flag = startage_7001_7002`), uses ICD-10 codes (`diag`), healthcare resource group codes (`hrg`), episode type (`epitype`), patient classification (`classpat`), admission method (`admimeth`) and neonatal care (`neocare`):
