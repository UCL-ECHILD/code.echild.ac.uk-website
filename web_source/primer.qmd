...
Each code list also has its idiosyncrasies. For example, in the [Hardelid et al](chc_hardelid_v1.qmd) [-@hardelid2014] list for chronic health conditions, some codes are only valid if the admission is at least 3 days long and others are only valid where the patient is at least ten years old. Each code list in the Repository is formatted in the same way, meaning that identifying these factors (which are also documented on this website) should be straightforward.

## Emergency admissions

One particular caveat to bear in mind is that some code lists are only designed for use with emergency admissions. This is the case, for example, with the lists of [adversity-related admissions](ari_herbert_v1.qmd) (@herbert2015) and stress-related presentations ([version 1](srp_blackburn_v1.qmd) (@blackburn2021) and [version 2](srp_nichobhthaigh_v2.qmd) (@nichobhthaigh2024)). You should therefore also consult the [emergency admissions](emergency_admissions_v1.qmd) code list that identifies HES admissions as such with reference to `admimeth` (admission methods) values `21` to `25`, `28`, and `2A` to `2D`. We recommend that you use the list available in the Repository as codes `25`, `2A` and `2D` were not available before 2013/14.
...
1. After identifying, and defining well, the target phenotype, the relevant coding system (e.g. ICD-10) is consulted to identify all candidate codes. This would include scouring all available codes, bearing in mind that relevant codes may appear in more than one chapter or under various sub-codes. Data other than diagnoses and procedures may also be relevant, such as in the [Zylbersztejn et al](births_zylb_v1.qmd) [-@zylbersztejn2020] list, which uses a combination of diagnostic and other information to identify maternity admissions.
...
While it is beyond the scope of the Repository to carry out this updating work, updated lists may be admitted to it. Compare, for example, the code list of stress-related emergency presentations developed by [Blackburn et al](srp_blackburn_v1.qmd) [-@blackburn2021] with that of [Ní Chobhthaigh et al](@nichobhthaigh2024) [-@nichobhthaigh2024]. The latter is an update of the former, accounting for the latest research and practice in paediatric mental health.
