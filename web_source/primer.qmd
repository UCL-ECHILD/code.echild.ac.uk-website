...
One particular caveat to bear in mind is that some code lists are only designed for use with emergency admissions. This is the case, for example, with the adversity-related admissions (@herbert2015) and stress-related presentations (@blackburn2021; @nichobhthaigh2024) lists. You should therefore also consult the [emergency admissions](emergency_admissions_v1.qmd) code list that identifies HES admissions as such with reference to `admimeth` (admission methods) values `21` to `25`, `28`, and `2A` to `2D`. We recommend that you use the list available in the Repository as codes `25`, `2A` and `2D` were not available before 2013/14.
...
Some code lists also use ICD-9 codes and/or ICD-10-PCS (procedure codes). Because these are not used in HES, and therefore not used in ECHILD, they are removed from code lists in this Repository. As it happens, the version of Feudtner et al's [-@feudtner2014] list in the Repository has had 1,097 codes removed and 194 codes truncated (all of which are documented according to our [design principles](principles.qmd)).
