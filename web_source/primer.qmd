...
Each code list also has its idiosyncrasies. For example, in the [Hardelid et al](chc_hardelid_v1.qmd) list for chronic health conditions, some codes are only valid if the admission is at least 3 days long and others are only valid where the patient is at least ten years old. Each code list in the Repository is formatted in the same way, meaning that identifying these factors (which are also documented on this website) should be straightforward.

## Emergency admissions

One particular caveat to bear in mind is that some code lists are only designed for use with emergency admissions. This is the case, for example, with the lists of [adversity-related admissions](ari_herbert_v1.qmd) and stress-related presentations ([version 1](srp_blackburn_v1.qmd) and [version 2](srp_nichobhthaigh_v2.qmd)). You should therefore also consult the [emergency admissions](emergency_admissions_v1.qmd) code list that identifies HES admissions as such with reference to `admimeth` (admission methods) values `21` to `25`, `28`, and `2A` to `2D`. We recommend that you use the list available in the Repository as codes `25`, `2A` and `2D` were not available before 2013/14.
