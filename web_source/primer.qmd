...
Currently within the Repository, we have a [range of code lists](code_list_index.qmd) that cover different target phenotypes. The Repository is fully searchable and you can consult the [index](code_list_index.qmd) for an overview of what is currently held.
...
You must be careful to recognise possible limitations in using code lists. First, when using administrative data, you must always consider the possibility of various biases such as that induced by the fact that patients admitted to hospital are generally in poorer health than those not admitted. This means you are less likely to detect all cases. Similarly, administrative hospital data, particularly HES inpatient records, will pick up fewer mental health presentations and other conditions primarily seen in outpatients or community settings.
