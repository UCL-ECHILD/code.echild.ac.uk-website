...
Each code list in the repository has (or will soon have) an associated R script and Stata do file that demonstrates one method of implementing each code list. You are, of course, free to use and adapt these scripts as you wish. Here we provide a brief guide on what each script does and we also provide an example of working with multiple phenotypes. Throughout, we try to demonstrate parsimonious coding, storing in memory as few objects as possible, themselves as small as possible, for example by dropping redundant flag variables and entire objects from memory when no longer needed. 

# How to find the scripts

The R scripts and Stata do files can be found on each code list page underneath the links to the code list files. Some lists may show "Not yet available" while we continue to develop the scripts.

![](download_script.png){fig-alt="A screenshot with a red arrow showing where to download a code list's R script."}
...
We are currently working on deployment of the Repository in the Secure Research Service. ECHILD users who need the scripts can e-mail [Matthew Jay](mailto:matthew.jay@ucl.ac.uk) to find out where they are.
...
6. Deal with flags. For example, in the [Hardelid et al list of chronic health conditions](chc_hardelid_v1.qmd), some codes are only valid if the admission length of stay is less than three days and others are only valid if the patient is aged at least 10 years. Generally, we deal with these conditions by setting our binary flags to false where these conditions are not met. Once we have done this, we drop all codes that are not validly in our code list.
