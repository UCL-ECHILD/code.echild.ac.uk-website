...
for (i in 1:nrow(code_lists)) {
  ...
  write(paste0("| **Code list file** | <href a=\"", id, ".csv\" download>", codelist_file, "</a> |"), fileConn, append = T)
  if (!is.na(restrictions_file)) {
    restrictions_filename <- paste0(id, "_restrictions.csv")
    write(paste0("| **Restrictions file ** | <href a=\"", restrictions_filename, ".csv\" download>", restrictions_file, "</a> |"), fileConn, append = T)
  } else {
  ...
  if (!is.na(truncations_file)) {
    ...
    write(paste0("| **Truncations file ** | <href a=\"", truncations_filename, ".csv\" download>", truncations_file, "</a> |"), fileConn, append = T)
  } else {
    write(paste0("| **Truncations file ** | NA |"), fileConn, append = T)
  }
  if (!is.na(removals_file)) {
    removals_filename <- paste0(id, "_removals.csv")
    write(paste0("| **Removals file ** | <href a=\"", removals_filename, ".csv\" download>", removals_file, "</a> |"), fileConn, append = T)
  } else {
