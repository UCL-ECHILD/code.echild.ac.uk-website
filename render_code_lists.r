...
for (i in 1:nrow(code_lists)) {
  ...
  write(paste0("| **Code list file (right-click to download)** | [", id, ".csv](", codelist_file, "){target=\"_blank\" download} |"), fileConn, append = T)
  if (!is.na(restrictions_file)) {
    restrictions_filename <- paste0(id, "_restrictions.csv")
    write(paste0("| **Restrictions file ** | [", restrictions_filename, "](", restrictions_file, "){target=\"_blank\" download} |"), fileConn, append = T)
  } else {
  ...
  if (!is.na(truncations_file)) {
    ...
    write(paste0("| **Truncations file ** | [", truncations_filename, "](", truncations_file, "){target=\"_blank\" download} |"), fileConn, append = T)
  } else {
    write(paste0("| **Truncations file ** | NA |"), fileConn, append = T)
  }
  if (!is.na(removals_file)) {
    removals_filename <- paste0(id, "_removals.csv")
    write(paste0("| **Removals file ** | [", removals_filename, "](", removals_file, "){target=\"_blank\" download} |"), fileConn, append = T)
  } else {
