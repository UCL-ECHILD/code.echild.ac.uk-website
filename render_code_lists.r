...
phenotype_repo_raw_content_prefix <- "https://raw.githubusercontent.com/UCL-ECHILD/phenotype-code-lists/main/"
...
for (i in 1:nrow(code_lists)) {
  ...
  write(paste0("| **Code list file** | <a href=\"", phenotype_repo_raw_content_prefix, codelist_file, "\" download>", id, "</a> |"), fileConn, append = T)
  if (!is.na(restrictions_file)) {
    restrictions_filename <- paste0(id, "_restrictions.csv")
    write(paste0("| **Restrictions file ** | <a href=\"", phenotype_repo_raw_content_prefix, restrictions_file, "\" download>", restrictions_filename, "</a> |"), fileConn, append = T)
  } else {
  ...
  if (!is.na(truncations_file)) {
    ...
    write(paste0("| **Truncations file ** | <a href=\"", phenotype_repo_raw_content_prefix, truncations_file, "\" download>", truncations_filename, "</a> |"), fileConn, append = T)
  } else {
    write(paste0("| **Truncations file ** | NA |"), fileConn, append = T)
  }
  if (!is.na(removals_file)) {
    removals_filename <- paste0(id, "_removals.csv")
    write(paste0("| **Removals file ** | <a href=\"", phenotype_repo_raw_content_prefix, removals_file, "\" download>", removals_filename, "</a> |"), fileConn, append = T)
  } else {
